...
function calculateDrag(velocity, crossSectionalArea, dragCoefficient, airDensity) {
 ...
 // 
 // Note: We calculate magnitude using v² (always positive) and then apply direction
 // separately in trajectory-engine.js to handle both upward and downward motion correctly.
 
 const dragForce = 0.5 * airDensity * velocity * velocity * dragCoefficient * crossSectionalArea;
 
 return {
 value: Math.abs(dragForce), // Always positive magnitude (v² makes it positive)
