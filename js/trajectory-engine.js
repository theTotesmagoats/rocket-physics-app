...
function createInitialState(rocketConfig, launchConditions) {
 // Calculate total initial mass correctly
 const rocketStructureMass = rocketConfig.rocket.mass;
 const propellantMass = rocketConfig.motor.propellantMass; // FIX: Use propellantMass instead of totalMass
 const parachuteMass = rocketConfig.parachute ? rocketConfig.parachute.mass : 0;
 const payloadMass = rocketConfig.payloadMass;
 
 const initialMass = rocketStructureMass + propellantMass + parachuteMass + payloadMass;
 
 console.log('Initial Mass:', (initialMass * 1000).toFixed(2), 'g');
 console.log('Rocket Structure:', (rocketStructureMass * 1000).toFixed(2), 'g');
 console.log('Propellant:', (propellantMass * 1000).toFixed(2), 'g');
 console.log('Parachute:', (parachuteMass * 1000).toFixed(2), 'g');
 console.log('Payload:', (payloadMass * 1000).toFixed(2), 'g');
 ...
 propellantMass: propellantMass, // Store for reference during burn calculations
...
function updateState(state, rocketConfig, launchConditions, deltaTime) {
 ...
 // Start with initial total mass and reduce as propellant burns
 const initialMass = rocketConfig.rocket.mass + state.propellantMass +
